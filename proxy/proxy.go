...
type NodeOperations interface {
	...
	RequestService(Request) interface{}
	Start() error
	Stop() error
}
...
func DeleteNode(node *Node) error {
	if _, err := http.Get("http://172.17.0.1:" + node.port); err != nil {
		log.Println(err)
		return err
	}
...
func (service Service) bindListeners(listenPort string, healthPath string) func(req *G.Request, resp *G.Message) {
	return func(req *G.Request, resp *G.Message) {
		...
		requester := fmt.Sprintf("http://172.17.0.1:%s%s", listenPort, service.Path)
...
func (node *Node) GetStatus(sync bool) (Status, error) {
	...
	addr := fmt.Sprintf("http://172.17.0.1:%s", node.port)
	log.Println(addr)
...
func (node *Node) RequestService(serviceRequest Request) interface{} {
